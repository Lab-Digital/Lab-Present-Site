...
{block name='main'}
  ...
  <form action="/admin/{$handle_url}" method="post" id="add_article">
    ...
    <div class="form_block">
      ...
      <textarea name="body" id="article_body">{$body|default:$article.news_body|default:''}</textarea>
    </div>
    <div class="form_block">
  ...
  {if !$isAdd|default:false}
    ...
    <div class="in text_in">
       ...
       <ul>
          {foreach from=$article.news_photos item=photo}<li><a href="/images/uploads/{$photo.name}_b.{$photo.ext}"><img src="/images/uploads/{$photo.name}_s.{$photo.ext}" /></a><button class="x" data="{$photo.name}" data-ext="{$photo.ext}">x</button><button class="url" data="{$server_root}/images/uploads/{$photo.name}_b.{$photo.ext}">URL</button></li>{/foreach}
       </ul>
    </div>
  {/if}
{/block}